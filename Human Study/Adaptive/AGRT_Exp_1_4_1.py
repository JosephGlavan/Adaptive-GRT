...
# IMPORTANT:
#    Versions < 2.X.X may not support p/a conditions from same file. Use the condition-appropriate file unless you are sure that it will work. 
...
MAX_TIME = 60   # Maximum duration of a trial (in seconds)
...
ITI = .5 # Time between end of last trial and start of referenceStimulus in adaptive block
...
class OrientedCircle (object):
    def __init__(self, win, orientation, rad, col, x, y):
        self._circleStim = visual.Circle(win=win, radius=rad, lineColor=col, fillColor=win.color, pos=[x,y], units='pix', colorSpace='rgb', opacity=1.0, lineWidth=STIM_LW)
        #self._lineStim = visual.Line(win=win, lineWidth=STIM_LW, lineColor=col, start=(x,y-rad), end=(x,y+rad), ori=orientation, colorSpace='rgb255')
        self._lineStim = visual.Line(win=win, lineWidth=STIM_LW, lineColor=col, start=(x + rad * cos(deg2rad(orientation)), y + rad * sin(deg2rad(orientation))), end=(x + rad * cos(deg2rad(orientation+180)), y + rad * sin(deg2rad(orientation+180))), colorSpace='rgb')
        self.pos = [x,y]
        self.radius = rad
        self.color = col
...
integralStimuli = [visual.Rect(win, units='pix', lineWidth=0, lineColor=None, fillColor=STIM_COLS[0], pos=(0, 0), size=STIM_SIZE, colorSpace='hsv'),
    visual.Rect(win, units='pix', lineWidth=0, lineColor=None, fillColor=STIM_COLS[1], pos=(0, 0), size=STIM_SIZE, colorSpace='hsv'),
    visual.Rect(win, units='pix', lineWidth=0, lineColor=None, fillColor=STIM_COLS[2], pos=(0, 0), size=STIM_SIZE, colorSpace='hsv'),
    visual.Rect(win, units='pix', lineWidth=0, lineColor=None, fillColor=STIM_COLS[3], pos=(0, 0), size=STIM_SIZE, colorSpace='hsv')]
...
def RunAgrtPracticeTrial(stimNum, feedback):
    """Runs a single trial with/out feedback and returns the response number"""
    # If we're using a single adaptive block (adapting both dimensions simultaneously) then it needs to return the response
    # If we're using two adaptive blocks (adapting to each dimension separately) then it only needs to return correct/incorrect
    global stimuli
    ...
    stimuli[stimNum].status = "NOT STARTED"
    stimuli[stimNum].setAutoDraw(False)
    referenceStimulus.status = "NOT STARTED"
    referenceStimulus.setAutoDraw(False)
    mask1status = "NOT STARTED"
    mask2status = "NOT STARTED"
    response = None; rt = -1
    RSOI = uniform(ONSET_RANGE[0], ONSET_RANGE[1])
    ...
    while continueRoutine:
        ...
        if t > MAX_TIME:
            continueRoutine = False
        # Start reference stimulus after ITI
        if t >= ITI and referenceStimulus.status == "NOT STARTED":
            referenceStimulus.setAutoDraw(True)
            referenceStimulus.status = "STARTED"
        # Turn off reference stimulus
        if t >= ITI + PROBE_DURATION and referenceStimulus.status == "STARTED":
            referenceStimulus.setAutoDraw(False)
            referenceStimulus.status = "FINISHED"
        # Start new mask as reference stimulus turns off
        if t >= ITI + PROBE_DURATION and mask1status == "NOT STARTED":
            mask = newMask()
            mask.setAutoDraw(True)
            mask1status = "STARTED"
        # Turn off first mask
        if t >= ITI + PROBE_DURATION + MASK_DURATION and mask1status == "STARTED":
            mask.setAutoDraw(False)
            mask1status = "FINISHED"
        # Start probe stimulus some random time (RSOI) after first mask turns off
        if t >= ITI + PROBE_DURATION + MASK_DURATION + RSOI and stimuli[stimNum].status == "NOT STARTED":
            stimuli[stimNum].setAutoDraw(True)
            stimuli[stimNum].status = "STARTED"
            responseClock.reset()
            event.clearEvents()
        # Turn off probe stimulus
        if t >= ITI + PROBE_DURATION + MASK_DURATION + RSOI + PROBE_DURATION and stimuli[stimNum].status == "STARTED":
            stimuli[stimNum].setAutoDraw(False)
            stimuli[stimNum].status = "FINISHED"
        # Start new mask and response guide as probe stimulus turns off
        if t >= ITI + PROBE_DURATION + MASK_DURATION + RSOI + PROBE_DURATION and mask2status=="NOT STARTED":
            mask = newMask()
            mask.setAutoDraw(True)
            mask2status = "STARTED"
            toggle(responseStimuli, True)
        # Turn off second mask
        if t >= ITI + PROBE_DURATION + MASK_DURATION + RSOI + PROBE_DURATION + MASK_DURATION and mask2status=="STARTED":
            mask.setAutoDraw(False)
            mask2status = "FINISHED"
        # Start listening for response when probe stimulus starts
        if t >= ITI + PROBE_DURATION + MASK_DURATION + RSOI and stimuli[stimNum].status != "NOT STARTED":
            keys = event.getKeys(keyList=['f','e','j','i'], timeStamped=responseClock)
            for k in keys:
                if rt < k[1]:
...
def RunAgrtTrial (stimulus, logfile=None, handler=None, trial=None, info=None, additionalArgs=None):
    ...
    # Check for break
    if trial % 200 == 1 and additionalArgs[-1] == 'main': # Offer a break every 200 trials during main block
        if trial==1: # Use the first trial as an opportunity to display instructions
            visual.TextStim(win=win, ori=0, name='breakInstructions',text="You will now complete the second part of the main experiment. You will periodically be offered a break.\n\n\nGoing forward, there will only be one object displayed.\n\nRemember:\n\nPress 'f' if the {obj1} is {dim1a} and {dim2a}.\n\nPress 'e' if the {obj1} is {dim1a} and {dim2b}.\n\nPress 'j' if the {obj1} is {dim1b} and {dim2a}.\n\nPress 'i' if the {obj1} is {dim1b} and {dim2b}.\n\n\nPress the spacebar when you are ready to continue...".format(obj=instrArgs[0], obj1=instrArgs[1], dim1a=instrArgs[2], dim1b=instrArgs[3], dim2a=instrArgs[4], dim2b=instrArgs[5], optfb=instrArgs[6]),
    ...
    if info['exp'] == 's':
        ...
        myStim = visual.Rect(win, units='pix', lineWidth=0, lineColor=None, fillColor=[STIM_HUE, stimX, stimY], pos=(0, 0), size=STIM_SIZE, colorSpace='hsv')
    else:
        raise RuntimeError("Experiment not recognized")
    ...
    if isinstance(handler, AGRT.AGRTHandler):
        t = 0; trialClock.reset(); continueRoutine = True
        while continueRoutine:
            t = trialClock.getTime()
            if t > MAX_TIME:
                continueRoutine = False
                # This might be a graceful way to repeat the trial but I am afraid of the recursion.
                #return  RunAgrtTrial(stimulus, logfile, handler, trial, info, additionalArgs)
                # Going to increase the MAX_TIME instead for now.
            # Start reference stimulus after ITI
            if t >= ITI and referenceStimulus.status == "NOT STARTED":
                referenceStimulus.setAutoDraw(True)
                referenceStimulus.status = "STARTED"
            ...
            if t >= ITI + PROBE_DURATION + MASK_DURATION and mask1status == "STARTED":
                ...
                mask1status = "FINISHED"
            # Start probe stimulus some random time (RSOI) after first mask turns off
            if t >= ITI + PROBE_DURATION + MASK_DURATION + RSOI and myStim.status == "NOT STARTED":
                myStim.setAutoDraw(True)
                myStim.status = "STARTED"
                responseClock.reset()
                event.clearEvents()
            # Turn off probe stimulus
            if t >= ITI + PROBE_DURATION + MASK_DURATION + RSOI + PROBE_DURATION and myStim.status == "STARTED":
                myStim.setAutoDraw(False)
            ...
            if response is not None and mask2status == "FINISHED":
                if feedback == True:
                    continueRoutine = False
                elif t >= ITI + PROBE_DURATION + MASK_DURATION + RSOI + PROBE_DURATION + MASK_DURATION + 1:
                    continueRoutine = False
        ...
        while continueRoutine:
            ...
            if t > MAX_TIME:
                continueRoutine = False
            # Start probe stimulus some random time (RSOI)
            ...
            if t >= RSOI + PROBE_DURATION and myStim.status == "STARTED":
                myStim.setAutoDraw(False)
                myStim.status = "FINISHED"
            # Start new mask as probe stimulus turns off
            if t >= RSOI + PROBE_DURATION and mask2status=="NOT STARTED":
                mask = newMask()
                mask.setAutoDraw(True)
                mask2status = "STARTED"
...
if expInfo['cond'] == 'a':
    ...
    finalInstructions = visual.TextStim(win=win, ori=0, name='InstructionsReminder',
        text="You will now complete a relatively short practice block with feedback.\n\n\nThere will be examples to help you remember:\n\nPress 'f' if the {obj1} is {dim1a} and {dim2a}.\n\nPress 'e' if the {obj1} is {dim1a} and {dim2b}.\n\nPress 'j' if the {obj1} is {dim1b} and {dim2a}.\n\nPress 'i' if the {obj1} is {dim1b} and {dim2b}.\n\n\nPress the spacebar when you are ready to begin...".format(obj=instrArgs[0], obj1=instrArgs[1], dim1a=instrArgs[2], dim1b=instrArgs[3], dim2a=instrArgs[4], dim2b=instrArgs[5], optfb=instrArgs[6]),
        font='Arial', pos=[0, 0], units="pix", height=INSTRUCTION_HEIGHT,wrapWidth=DISPLAY_DIMENSIONS[0]-100, anchorVert='center',
        color='black', colorSpace='rgb', opacity=1, depth=0.0)#, anchorHoriz='left')
    ...
    if expInfo['exp'] == 's':
        ...
        dim1Extremes = _SIZE
        dim2Extremes = _ORI
        responseStimuli = [OrientedCircle(win, STIM_ORI[0], STIM_SIZES[0], STIM_COL, -.4*DISPLAY_DIMENSIONS[0], -.4*DISPLAY_DIMENSIONS[1]),
            OrientedCircle(win, STIM_ORI[1], STIM_SIZES[1], STIM_COL, -.4*DISPLAY_DIMENSIONS[0], .4*DISPLAY_DIMENSIONS[1]),
            OrientedCircle(win, STIM_ORI[2], STIM_SIZES[2], STIM_COL, .4*DISPLAY_DIMENSIONS[0], -.4*DISPLAY_DIMENSIONS[1]),
            OrientedCircle(win, STIM_ORI[3], STIM_SIZES[3], STIM_COL, .4*DISPLAY_DIMENSIONS[0], .4*DISPLAY_DIMENSIONS[1]),
            visual.TextStim(win=win, text="f", color="black", units="pix", anchorVert='center', height=INSTRUCTION_HEIGHT, pos=(-.35*DISPLAY_DIMENSIONS[0], -.4*DISPLAY_DIMENSIONS[1])),
            visual.TextStim(win=win, text="e", color="black", units="pix", anchorVert='center', height=INSTRUCTION_HEIGHT, pos=(-.35*DISPLAY_DIMENSIONS[0], .4*DISPLAY_DIMENSIONS[1])),
            visual.TextStim(win=win, text="j", color="black", units="pix", anchorVert='center', height=INSTRUCTION_HEIGHT, pos=(.3*DISPLAY_DIMENSIONS[0], -.4*DISPLAY_DIMENSIONS[1])),
            visual.TextStim(win=win, text="i", color="black", units="pix", anchorVert='center', height=INSTRUCTION_HEIGHT, pos=(.3*DISPLAY_DIMENSIONS[0], .4*DISPLAY_DIMENSIONS[1]))]
        referenceStimulus = OrientedCircle(win, np.mean(_ORI), np.mean(_SIZE), STIM_COL, 0, 0)
    elif expInfo['exp'] == 'i':
        stimuli = integralStimuli
        dim1Extremes = STIM_SAT
        dim2Extremes = STIM_VAL
        responseStimuli = [visual.Rect(win, units='pix', lineWidth=0, lineColor=None, fillColor=STIM_COLS[0], pos=(-.4*DISPLAY_DIMENSIONS[0], -.4*DISPLAY_DIMENSIONS[1]), size=STIM_SIZE, colorSpace='hsv'),
            visual.Rect(win, units='pix', lineWidth=0, lineColor=None, fillColor=STIM_COLS[1], pos=(-.4*DISPLAY_DIMENSIONS[0], .4*DISPLAY_DIMENSIONS[1]), size=STIM_SIZE, colorSpace='hsv'),
            visual.Rect(win, units='pix', lineWidth=0, lineColor=None, fillColor=STIM_COLS[2], pos=(.4*DISPLAY_DIMENSIONS[0], -.4*DISPLAY_DIMENSIONS[1]), size=STIM_SIZE, colorSpace='hsv'),
            visual.Rect(win, units='pix', lineWidth=0, lineColor=None, fillColor=STIM_COLS[3], pos=(.4*DISPLAY_DIMENSIONS[0], .4*DISPLAY_DIMENSIONS[1]), size=STIM_SIZE, colorSpace='hsv'),
            visual.TextStim(win=win, text="f", color="black", units="pix", anchorVert='center', height=INSTRUCTION_HEIGHT, pos=(-.3*DISPLAY_DIMENSIONS[0], -.4*DISPLAY_DIMENSIONS[1])),
            visual.TextStim(win=win, text="e", color="black", units="pix", anchorVert='center', height=INSTRUCTION_HEIGHT, pos=(-.3*DISPLAY_DIMENSIONS[0], .4*DISPLAY_DIMENSIONS[1])),
            visual.TextStim(win=win, text="j", color="black", units="pix", anchorVert='center', height=INSTRUCTION_HEIGHT, pos=(.3*DISPLAY_DIMENSIONS[0], -.4*DISPLAY_DIMENSIONS[1])),
            visual.TextStim(win=win, text="i", color="black", units="pix", anchorVert='center', height=INSTRUCTION_HEIGHT, pos=(.3*DISPLAY_DIMENSIONS[0], .4*DISPLAY_DIMENSIONS[1]))]
        referenceStimulus = visual.Rect(win, units='pix', lineWidth=0, lineColor=None, fillColor=[STIM_HUE, np.mean(STIM_SAT), np.mean(STIM_VAL)], pos=(0,0), size=STIM_SIZE, colorSpace='hsv')
    else:
        raise RuntimeError("Experiment not recognized")
    
    Instructions()
    for block in range(2): #range(2):
        shuffle(trialOrder)
        for trial in trialOrder:
            trialCounter += 1
            RunAgrtPracticeTrial(trial, True)
    toggle(responseStimuli, False)

    visual.TextStim(win=win, ori=0, name='InstructionsReminder',
        text="You will now complete the first part of the main experiment. There will no longer be feedback or examples to help you.\n\n\nCompare the two objects and remember:\n\nPress 'f' if the second {obj1} is {dim1a} and {dim2a}.\n\nPress 'e' if the second {obj1} is {dim1a} and {dim2b}.\n\nPress 'j' if the second {obj1} is {dim1b} and {dim2a}.\n\nPress 'i' if the second {obj1} is {dim1b} and {dim2b}.\n\n\nPress the spacebar when you are ready to begin...".format(obj=instrArgs[0], obj1=instrArgs[1], dim1a=instrArgs[2], dim1b=instrArgs[3], dim2a=instrArgs[4], dim2b=instrArgs[5], optfb=instrArgs[6]),
        font='Arial', pos=[0, 0], units="pix", height=INSTRUCTION_HEIGHT,wrapWidth=DISPLAY_DIMENSIONS[0]-100, anchorVert='center',
    ...
    with open(file.rpartition(".")[0]+"_burnin.txt", 'a') as burnFile:
        burnFile.write("Version\tDate\tSubject\tExperiment\tCondition\tBlock\tTrial\tDim1\tDim2\tStimulus\tStimNum\tKey\tResponse\tRT\tCorrect\tLambda\tStimEst\n")
        AGRT.RunAdaptiveBlock(trialFun = RunAgrtTrial, numTrials = 8, Xrange = dim1Extremes, Yrange = dim2Extremes,
                              Xsteps = 100, Ysteps = 100, lapseRate = 0.08, overallAcc = ACCURACY_CRITERION, savePosterior = True,
                              logfile = burnFile, additionalArgs = [False], info=expInfo)
    
    AGRT.RunAdaptiveGRTExperiment(trialFunction = RunAgrtTrial, nAdaptiveTrials = 144, nGRTtrials = NUM_TRIALS, #8, nGRTtrials = 8, #
                                  dim1range = dim1Extremes, dim2range = dim2Extremes, 
                                  dim1steps=100, dim2steps=100, lapse=0.08, 
    ...
    for block in range(NUM_BLOCKS):
        ...
        if block % 25 == 0 and block != 0 and block != NUM_TRIALS: # Offer a break every 200 trials
            visual.TextStim(win=win, ori=0, name='breakInstructions',text="You may now take a short break.\n\nPress the spacebar when you are ready to continue.",
            font='Arial', pos=[0, 0], units="pix", height=INSTRUCTION_HEIGHT,wrapWidth=DISPLAY_DIMENSIONS[0]-100, anchorVert='center', color='black', colorSpace='rgb', opacity=1, depth=0.0).draw()
            win.flip()
            while event.getKeys('space') == []:
