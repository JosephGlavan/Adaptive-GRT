...
# File containing the PsiHandler class for AGRT. 
# Eventually might contain: 
# - AGRT-Handler class to run an entire experiment
# - Full subject model
...
#            self._probResponseGivenLambdaX = (1-self._r) + (2*self._r-1) * (stats.norm.cdf(self._x, self._alpha, self._beta)*(1-self.delta)+self.delta/2)
    ...
    def update(self, response=None):
        ...
        #Create H(x, r)
        self._entropyXResponse = -1* sum(self._probLambdaGivenXResponse * np.log10(self._probLambdaGivenXResponse), axis=(1,2)).reshape((len(self.r),1,1,len(self.x)))
    ...
    def estimateThreshold(self, thresh, lam):
        if lam is None:
            ...
            lamb = lam
        def of1 (x):
        	(stats.norm.cdf(lamb[0], loc=x, scale=lamb[1]) - np.sqrt(thresh))**2
        def of2 (x):
        	((1 - stats.norm.cdf(lamb[0], loc=x, scale=lamb[1])) - np.sqrt(thresh))**2
        return (optimize.minimize_scalar(of1).x, optimize.minimize_scalar(of2).x)
#        if self._TwoAFC:
...
class AGRTHandler (StairHandler):
    ...
    def __init__(self,
        ...
        def findMaxBeta (x, dimRange):
            (stats.norm.cdf(np.average(dimRange), loc=dimRange[0], scale=x) - .95)**2
        dim1betaRange = (0, optimize.minimize_scalar(findMaxBeta, dim1range).x)
        dim1betaRange[0] = dim1betaRange[1]/dim1steps
        dim2betaRange = (0, optimize.minimize_scalar(findMaxBeta, dim2range).x)
    ...
    def estimateGRTintensities(self, overallAccuracy, lambdas=None):
        ...
        try:
            if len(lambdas) != 2:
                msg = ("Invalid user-specified lambdas tuple. New estimates for lambda will be computed.")
                warnings.warn(msg, SyntaxWarning)
                lambdas = (None, None)
            else:
                for i in range(len(lambdas)):
                    if len(lambdas[i]) != 2:
                        msg = ("Invalid user-specified lambda pair. A new estimate of lambda will be computed.")
                        warnings.warn(msg, SyntaxWarning)
                        lambdas[i] = None
        except TypeError:
            msg = ("Invalid user-specified lambdas tuple. New estimates for lambda will be computed.")
...
def RunAdaptiveBlock (trialFun, numTrials, Xrange, Yrange, Xsteps=100, Ysteps=100, lapseRate=0.0, overallAcc=0.75, optionalArgs=None, postFun=None):
    """
    ...
    agrt = AGRTHandler(nTrials=numTrials, delta1=lapseRate, delta2=lapseRate, dim1range=Xsteps, dim2range=Ysteps)
    
    for intens in agrt:
        result = trialFun(intens, optionalArgs)
...
class GRTHandler (TrialHandler):
    ...
    def __init__(self, nTrials, blockingFactor=2):
        ...
        TrialHandler.__init__(trialList = ((0,0),(1,0),(0,1),(1,1)) * blockingFactor, 
                              numTrials = nTrials / 4 / blockingFactor, 
                              method='random')
...
def RunGRTBlock (trialFun, numTrials, stimuli, blockFactor=2, optionalArgs=None, postFun=None):
    """
    ...
    for stim in grt:
        result = trialFun((stimuli[0][stim[0]], stimuli[1][stim[1]]), optionalArgs)
        if result is None: # or whatever value I want to use as the graceful escape code
            break
        else:
            if postFun is not None:
...
def RunAdaptiveGRTExperiment (trialFunction, nAdaptiveTrials, nGRTtrials, 
                              ...
                              adaptiveFunArgs=None, adaptivePostFun=None, 
                              blockingFactor=2, grtFunArgs=None, grtPostFun=None):
    """
    ...
    RunGRTBlock(trialFun=trialFunction, numTrials=nGRTtrials, 
                stimuli=RunAdaptiveBlock(trialFun=trialFunction, numTrials=nAdaptiveTrials, 
                                         ...
                                         optionalArgs=adaptiveFunArgs, postFun=adaptivePostFun), 
                blockFactor=blockingFactor, optionalArgs=grtFunArgs, postFun=grtPostFun)
...
def GRTSubjectModel (x,y,delta,epsilon,sig,m,n,rho, R=[0,1,2,3]):
    """
    ...
    return np.take(np.array([mvnun(np.array([-np.inf, -np.inf]), tau, mymean, mycov)[0],
                             ...
                             mvnun(tau, np.array([np.inf, np.inf]), mymean, mycov)[0]]), R)
